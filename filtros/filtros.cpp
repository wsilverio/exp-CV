...
int main(void){
    ...
    for (int i = 0, m[] = {3, 5, 9, 15, 35, 75, 105, (img.cols%2)?img.cols:img.cols-1}; i < sizeof(m)/sizeof(int); ++i){
        ...
        imshow("filtro " + to_string(m[i]) + "x" + to_string(m[i]), result);
        imwrite("filtro_" + to_string(m[i]) + "x" + to_string(m[i]) + ".png", result);

        imshow("img - filtro" + to_string(m[i]) + "x" + to_string(m[i]), img - result);
        imwrite("img_-_filtro" + to_string(m[i]) + "x" + to_string(m[i]) + ".png", img - result);
    }
    ...
    imshow("filtro_Placa 3x3", result);
    imwrite("filtro_Placa_3x3.png", result);

    imshow("img - filtro_Placa 3x3", img - result);
    imwrite("img_-_filtro_Placa 3x3.png", img - result);
